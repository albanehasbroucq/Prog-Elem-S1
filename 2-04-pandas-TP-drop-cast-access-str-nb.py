...
# %%
# votre code
df = pd.read_csv('data/objects-on-the-moon.csv') #on lit le doc au format csv avec pandas
print(df.ndim) #j'affiche sa dimension
print(df.shape) # j'afficeh sa taille
df.head() #j'affiche quelques premieres lignes (5 par défaut de head)
...
# %% [markdown]
# On remarque que la colonne Size est entièrement vide
...
# %%
# votre code
df.dropna(axis=1,how='all', inplace=True) #avec axis on indique qu'on veut supprimer les colonnes et avec how, on précise
#qu'on les supprime que si toutes les valeurs sont manquantes
#inplace est mis à True pour que la DataFrame soit directement modifiée
#on vérifique qu'on a bien enelevé la colonne 'Size' en affichant qq lignes :
df.head()
...
# %% [markdown]
# On remarque que cette ligne ne contient aucune donnée

# %%
df.dropna(how='all', inplace=True) #axis=0 est déjà mis par défaut donc on agit sur les lignes, et comme avant on précise how='all'
#vérification :
df
...
# %% [markdown]
# On remarque que le type de la colonne masse est un objet alors qu'elle contient des chiffres, ce qui n'est pas logique
...
# %% [markdown]
# on remarque que, parfois, la masse donnée est une borne (par exemple : '<12626'), d'où l'utilisation de str plutôt
# que des floatants 
...
# %%
# votre code
df['Mass (lb) orig'] = df['Mass (lb)'] #on conserve la colonne de masse
df['Mass (lb)'] = pd.to_numeric(df['Mass (lb)'], errors='coerce') #on précise que les valeurs ne pouvant pas etre changées
#doivent etre remplacées par la valeur manquante NaN
#vérification:
df['Mass (lb)'].dtype
